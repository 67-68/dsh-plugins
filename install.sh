...
# 0e) DSH web-app 架构守护：dsh-web-app bundle 默认禁用 host plane 的
#     compaction-basic（compaction backend 移到 preset 层），但 dsh-mode-gate
#     在 host plane 上 inject 了 compaction 服务，host plane 缺它会导致插件树
#     加载失败（"dsh-mode-gate: pending (waiting for service: compaction)"）。
...
# 3b) ankify 工具链已抽离到独立的 sibling 仓库 ankify_tools（含自己的
