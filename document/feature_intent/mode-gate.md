...
- 额外申请：`dev_tool_search`（或旧名 `request_extra`）无参时查看当前能力，带参时作为问题向用户申报，批准后追加到当前 session。
- bash 禁止列表（deny-list）：全局生效，命中即拒绝；默认禁止 `curl` / `wget`，网页阅读统一走 `read_url` 系列工具。
- 模式切换：AI 通过 `switch_mode` 请求，用户批准后生效；`declare_target` 携带有效 `mode` 时同步写入当前模式。切换模式不重置已声明的 skills/bash。

## 规则

- 除 `declare_target` / `switch_mode` / `skill_search` / `dev_tool_search` / `request_extra` / 少量白名单工具外，未声明 Target 的工具调用一律拒绝。
...
- bash 命令按管道/分隔符切分后逐段检查动词：忽略 `for`/`do`/`done` 等 shell 关键字和 heredoc 正文，解析 `$(...)` / 反引号内的命令动词；先匹配禁止列表，再检查是否在已声明 bash 动词内（`echo`/`printf`/`pwd`/`cd`/`true`/`false` 为内置只读动词，免声明），最后按模式做安全分类。
- 被 bash 白名单拦截时，deny 原因必须指明缺失的命令动词，并提示用 `dev_tool_search`（或 `request_extra`）申请或 `declare_target` 补充。
